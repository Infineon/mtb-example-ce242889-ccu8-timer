# PSOC&trade; Control C1 MCU: CCU8 timer

This code example demonstrates the use of the capture and compare unit 8 (CCU8) in the PSOC&trade; Control C1 MCU in timer mode.

The CCU8 timer slice program blinks an LED when a timer period matches an interrupt.

[View this README on GitHub.](https://github.com/Infineon/mtb-example-ce242889-ccu8-timer)

[Provide feedback on this code example.](https://yourvoice.infineon.com/jfe/form/SV_1NTns53sK2yiljn?Q_EED=)
...
The following example clones the "[CCU8 timer](https://github.com/Infineon/mtb-example-ce242889-ccu8-timer)" application with the desired name "CCU8 timer" configured for the *KIT_PSC1M3_EVK* BSP into the specified working directory, *C:/mtb_projects*:

   ```
   project-creator-cli --board-id KIT_PSC1M3_EVK --app-id mtb-example-ce242889-ccu8-timer --user-app-name CCU8Timer --target-dir "C:/mtb_projects"
   ```
...
This example shows the working of the CCU8 peripheral of the PSOC&trade; Control C1 MCU. It consists of one slice configured as timer and an interrupt function - 'TIMER_0_PERIOD_MATCH_EVENT_HANDLER()'. This interrupt function toggles an LED in the main function if there is a time lapse and the period match event is triggered.
...
| :------- | :------------    | :------------ |
| CCU8 (PDL) | CCU80 | Generates timer period match interrupts |
| CCU8 (PDL) | TIMER0 | Generates period match interrupts to blink the LED at 1 Hz |
...
Document title: *CE242889* – *PSOC&trade; Control C1 MCU: CCU8 timer*
